...
if ($search_option === 'title') {
  $sql = "SELECT * FROM inform_board WHERE title LIKE '%$keyword%'";
} elseif ($search_option === 'content') {
  $sql = "SELECT * FROM inform_board WHERE content = '$keyword'";
} elseif ($search_option === 'name') {
  $sql = "SELECT * FROM inform_board WHERE name LIKE '%$keyword%'";
} else {
  // 유효하지 않은 검색 조건인 경우 기본 쿼리를 실행
  $sql = "SELECT * FROM inform_board";
}?>
...
<td width="500"><a href="inform_detail.php?idx=<?php echo $board["idx"];?>"><?php echo $title;?></a></td>
