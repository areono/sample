...
if ($search_option === 'title') {
  $sql = "SELECT * FROM qna_board WHERE title LIKE '%$keyword%' AND lock_post = 0"; // 비밀글 제외
} elseif ($search_option === 'content') {
  $sql = "SELECT * FROM qna_board WHERE content = '$keyword' AND lock_post = 0"; // 비밀글 제외
} elseif ($search_option === 'name') {
  $sql = "SELECT * FROM qna_board WHERE name LIKE '%$keyword%' AND lock_post = 0"; // 비밀글 제외
} else {
...
?>
    <tr class="list_board">
        ...
        <td width="500"><a href="qna_detail.php?idx=<?php echo $board["idx"];?>"><?php echo $title;?></a></td>
