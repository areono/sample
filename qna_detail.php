...
<body>
	<?php
		...
		if (!isset($_COOKIE['hit_count_' . $bno])) {
			...
			$hit = mysqli_fetch_array(mc("SELECT view FROM qna_board WHERE idx ='".$bno."'"));
			$new_hit = $hit['view'] + 1;
			mc("UPDATE qna_board SET view = '".$new_hit."' WHERE idx = '".$bno."'");
		}

		$hit = mysqli_fetch_array(mc("SELECT * from qna_board where idx ='".$bno."'"));
		$sql = mc("SELECT * from qna_board where idx='".$bno."'"); /* 받아온 idx값을 선택 */
...
<div id="board_read">
	...
	<div id="bo_ser">
        <ul>
            ...
            ?>
                        ...
                        <?php if (!$has_reply) : ?>
                            <li><a href="qna_modify.php?idx=<?php echo $board['idx']; ?>">수정</a></li>
                        <?php else : ?>
                        ...
                        <?php else : ?>
                            <li><a href="qna_delete.php?idx=<?php echo $board['idx']; ?>">삭제</a></li>
                        <?php endif; ?>
                <?php } ?>
            </ul>
        <?php } ?>
	...
	<h3>답변 목록</h3>
		<?php
			...
			$sql3 = mc("SELECT * from qna_comment where board_idx='$bno' ORDER BY idx DESC");
		...
		<div class="dap_lo">
			...
			<!-- 댓글 수정 폼 dialog -->
			<div class="dat_edit">
				<form method="post" action="qna_comment_modify.php">
			...
			<div class='dat_delete'>
				<form action="qna_comment_delete.php" method="post">
					<input type="hidden" name="rno" value="<?php echo $reply['idx']; ?>" /><input type="hidden" name="b_no" value="<?php echo $bno; ?>">
...
<!-- 댓글 입력 폼 -->
	<div class="dap_ins">
		...
		<?php else : ?>
			<form action="qna_comment_ok.php?idx=<?php echo $bno; ?>" method="post">
