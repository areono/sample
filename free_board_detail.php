...
<body>
	<?php
		...
		$hit = mysqli_fetch_array(mc("SELECT * from free_board where idx ='".$bno."'"));
		$hit = $hit['view'] + 1;
		$fet = mc("UPDATE free_board set view = '".$hit."' where idx = '".$bno."'");
		$sql = mc("SELECT * from free_board where idx='".$bno."'"); /* 받아온 idx값을 선택 */
		...
		if (isset($_POST['recommend'])) {
			...
			try {
				// 이미 해당 게시물을 추천했는지 확인
				$result = mc("SELECT * FROM member WHERE id = '$user_id' AND post_id = '$bno'");
		
				if ($result->num_rows === 0) {
					// 해당 게시물에 추천 정보 추가
					mc("INSERT INTO member (id, passwd) VALUES ('$user_id', '$bno')");
		
					// 추천 수 증가
					$like = $board['like_count'] + 1;
					mc("UPDATE free_board SET like_count = '$like' WHERE idx = '$bno'");
...
<div id="board_read">
	...
	<h3>댓글목록</h3>
		<?php
			...
			$sql3 = mc("SELECT * from free_comment where board_idx='$bno' ORDER BY idx DESC");
